...
## OBSERVATION :  X1 and X2 contribute significantly to SSReg
######## What is the contribution levels of all variables ############
####### Evaluation Criteria
###### 1. minimum MSres(p) and maximum R2adj(p) are equivalent.
#         In general, the value of p corresponding to maximum value of R2 adj( p ) is chosen for the subset model.
#      2. Mallow’s C p statistics:   the models with smallest Cp are considered to be better
#                                    than those models with higher.
#      3. Akaike’s information criterion (AIC) : (GLM)
#      4. Bayesian information criterion (BIC) : A model with smaller value of BIC is preferable.
#      5. PRESS statistic: A subset regression model with smaller value of PRESS ( p ) is preferable.
################### GENERAL COMMENTS
#1. None of the methods among forward selection, backward elimination or stepwise regression
#guarantees the best subset model.
#2.The order in which the explanatory variables enter or leave the models does not indicate the
#order of importance of explanatory variable.
...
### RECOMMENDATION : X4,X1,X2,X3; X4 + X1 ; 96%
...
### RECOMMENDATION : X4 : 67%; X1+X2 : 97%; X1+X2+X3+X4 = 98%
...
###### Sample function that loops t.test between X-var and Y
fst <- function(x) {
  htest_res <- t.test(x,Y) 
  htest_res
  #htest_res$p.value 
  #htest_res$statistic
  #htest_res$alternative
}
lapply(haldcement[,-1], fst)

######### Result : P is low for all tests, so Null blows. Accept H1.
###########Result : Significant difference in means between X1,X2,X3 and X4
########## T-statistic in descending order : X2 (-7.8744), X4 (-10.481),X3 (-18.447),X1 (-19.634)

########Recommendation: t-test: X2 and X4
